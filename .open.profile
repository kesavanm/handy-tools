...
#LET FOLLOW WHITE RABBIT - cMATRIX
...
#CROSS SALE `HANDY-TOOLS`
function install_handy_tools(){
  echo "you may also want to install the handy-tools for friendly experience"
  echo "(y)es , I want it... or sorry (n)ot now : "
  read $choice
...
function git_stat(){
    line=$(border `echo 25+5+7+7+7+6|bc`)		# name file add del 5x|'s

    echo "$line"
    printf "|%25s|%5s|%7s|%7s|%7s| \n" name files insert delete commit; 
    echo "$line"

    for name in "Kesavan Muthuvel" "kmuthuvel" \
                 "Manjil" "Manjil ThapaMagar"
    do 
        printf "|%25s|%5d" "$name" $(git shortlog -sn --author="$name"| awk '{s+=$1} END {print s}')
        git log --shortstat --since="1 Jan, 2013" --author="$name" | 
        grep -E "fil(e|es) changed" | 
        awk '{files+=$1; inserted+=$4; deleted+=$6} END  {printf "|%7d|%7d|%7d|\n",  files,  inserted,  deleted   }'; 
    done ; 
    echo "$line"
}
